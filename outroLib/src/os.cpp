...
const CBYTE    *OS_application_name = "Urban Chaos credits";
...
OS_Texture *OS_texture_create(const CBYTE *fname, SLONG invert)
{
	SLONG format;
		...
		ULONG       draw)
{
...

	{
		...
		if (d3d->ValidateDevice(&num_passes) != D3D_OK)
		{
			...
			//OS_string((CBYTE*)"Validation failed: draw = 0x%x\n", draw);
		}
	}
