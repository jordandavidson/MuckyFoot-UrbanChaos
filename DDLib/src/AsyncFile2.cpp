...
{
	// tell thread to die
	TRACE((CBYTE*)"MAIN: Telling worker thread to die\n");
	...
	TRACE((CBYTE*)"MAIN: Waiting for worker thread to die\n");
	EnterCriticalSection(&csLock);
	while (KillThread != 2)
	{
		LeaveCriticalSection(&csLock);
...
DWORD WINAPI ThreadRun(LPVOID arg)
{
	for (;;)
	{
		...
		if (KillThread == 1)
		{
			TRACE((CBYTE*)"Worker: Received kill instruction\n");
			KillThread = 2;
			LeaveCriticalSection(&csLock);
			return 0;
		}
		...
		if (CancelKey)
		{
			TRACE((CBYTE*)"Worker: Received cancel instruction\n");
		...
		{
			TRACE((CBYTE*)"Worker: Received file load instruction\n");
		...
		if (file)
		{
			...
			// read file
			TRACE((CBYTE*)"Worker: Commence reading file\n");

			while (file->blen > BytesPerMillisecond)
			{
				TRACE((CBYTE*)"Worker: Reading block\n");
			...
			if (file->blen)
			{
				TRACE((CBYTE*)"Worker: Reading final block\n");
				ReadFile(file->hFile, file->buffer, file->blen, &amount, NULL);
			}
			TRACE((CBYTE*)"Worker: Complete\n");
...
bool LoadAsyncFile(char* filename, void* buffer, DWORD blen, void* key)
{
	...
	// open the file
	file->hFile = CreateFileA (filename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL);
	...
	TRACE((CBYTE*)"MAIN: Adding block to active list\n");
	EnterCriticalSection(&csLock);
	ActiveLink(file);
	LeaveCriticalSection(&csLock);

	TRACE((CBYTE*)"MAIN: Signalling worker thread\n");
	SetEvent(hEvent);
...
void CancelAsyncFile(void* key)
{
	...
	TRACE((CBYTE*)"MAIN: Instructing worker thread to cancel\n");
	EnterCriticalSection(&csLock);
	CancelKey = key;
	LeaveCriticalSection(&csLock);
	...
	TRACE((CBYTE*)"MAIN: Continuing after cancel\n");
}
