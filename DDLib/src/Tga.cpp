...
TGA_Info TGA_load_psx(const CBYTE *file,SLONG        max_width,SLONG        max_height,UBYTE   *data,UBYTE *pal)
{
	...
	if (handle == NULL)
	{
		psx_load_error((CBYTE*)"dont exist",file);
	...
	{
		psx_load_error((CBYTE*)"no pal/greyscale",file);
	...
	if (tga_width  > max_width ||
		tga_height > max_height)
	{
		psx_load_error((CBYTE*)"funny size",file);
	...
	else
	{
		if(tga_image_type!=3)
		{
			psx_load_error((CBYTE*)"not gray scale",file);
  ...
  file_error:;
		psx_load_error((CBYTE*)"Mystery file error",file);
