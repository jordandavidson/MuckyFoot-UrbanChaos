...
{
	D3DDeviceInfo	*new_device, *next_best_device = nullptr;
								...
								)
{
	...
	// Copy Name
	if (!lpD3DName)
		szTemp = LPTSTR("UNKNOWN");//TEXT("UNKNOWN");
    else
        szTemp	=	lpD3DName;
...
void D3DDeviceInfo::CheckCaps(LPDIRECT3DDEVICE3 the_device)
{
	...
	if (hw.dpcTriCaps.dwTextureBlendCaps & D3DPTBLENDCAPS_MODULATEALPHA)
	{
		...
		TRACE((CBYTE*)"Card can do MODULATEALPHA\n");
	}
	else
	{
		CanDoModulateAlpha = false;
		TRACE((CBYTE*)"Card *cannot* do MODULATEALPHA\n");
	}

	if (hw.dpcTriCaps.dwDestBlendCaps & D3DPBLENDCAPS_INVSRCCOLOR)
	{
		CanDoDestInvSourceColour = true;
		TRACE((CBYTE*)"Card can do INVSRCCOLOR\n");
	}
	else
	{
		CanDoDestInvSourceColour = false;
		TRACE((CBYTE*)"Card *cannot* do INVSRCCOLOR\n");
	}
		...
		(hw.dpcTriCaps.dwSrcBlendCaps & D3DPBLENDCAPS_DESTCOLOR))
	{
		...
		TRACE((CBYTE*)"Card can do ADAMI LIGHTING\n");
	}
	else
	{
		CanDoAdamiLighting = false;
		TRACE((CBYTE*)"Card *cannot* do ADAMI LIGHTING\n");
	}

	SLONG adami_lighting = ENV_get_value_number((CBYTE*)"Adami_lighting", -1, (CBYTE*)"Render");

	if (adami_lighting == -1)
	{
		ENV_set_value_number((CBYTE*)"Adami_lighting", 1, (CBYTE*)"Render");
	...
	if (adami_lighting == 0)
	{
		...
		TRACE((CBYTE*)"Overriding ADAMI LIGHTING\n");
	}
#endif
...
HRESULT	DDDriverInfo::LoadDevices(LPDIRECT3D3 lpD3D3)
{
	...
	if(!DevicesLoaded())
	{
		...
		result	=	lpD3D3->EnumDevices((LPD3DENUMDEVICESCALLBACK)DeviceEnumCallback, &callback_info);
		if(FAILED(result))
		{
			return	result;
		}
