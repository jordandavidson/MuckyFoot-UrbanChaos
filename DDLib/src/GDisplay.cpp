...
#include "..\..\DDEngineLib\include\poly.h"
#include "..\..\DDEngineLib\include\vertexbuffer.h"

#include "..\..\DDEngineLib\include\polypoint.h"
#include "..\..\DDEngineLib\include\renderstate.h"
#include "..\..\DDEngineLib\include\polypage.h"
#include "..\..\DDEngineLib\include\panel.h"
//#include "poly.h"
...
extern HINSTANCE	hGlobalThisInst;
	...
	VideoRes = ENV_get_value_number((CBYTE*)"video_res", -1, (CBYTE*)"Render");
...
void Display::RunFMV()
{
	...
	// should we run it?
	if (!ENV_get_value_number((CBYTE*)"play_movie", 1, (CBYTE*)"Movie"))	return;
		...
		SLONG *shift)
{
		...
		num_bits  ==  0)
	{
		...
		TRACE((CBYTE*)"No valid masks and shifts.\n");
		LogText((CBYTE*)"No valid masks and shifts.\n");
...
HRESULT	Display::InitFront(void)
{
	...
	{
		// Error
		DebugText((CBYTE*)"InitFront: unable to create front surface result %d\n",result);
		return	result;
	}
	...
	{
		lp_DD_GammaControl = NULL;
		TRACE((CBYTE*)"No gamma\n");
	}
	else
	{
		TRACE((CBYTE*)"Gamma control OK\n");
...
void *Display::screen_lock(void)
{
	if (DisplayFlags & DISPLAY_LOCKED)
	{
		...
		TRACE((CBYTE*)"Locking the screen when it is already locked!\n");
	}
	else
	{
...
void Display::PlotPixel(SLONG x, SLONG y, UBYTE red, UBYTE green, UBYTE blue)
{
	...
	else
	{
		...
		TRACE((CBYTE*)"PlotPixel while screen is not locked.\n");
	}
}
...
void Display::PlotFormattedPixel(SLONG x, SLONG y, ULONG colour)
{
	...
	else
	{
		...
		TRACE((CBYTE*)"PlotFormattedPixel while screen is not locked.\n");
	}
}
...
void Display::SetGamma(int black, int white)
{
	...
	ENV_set_value_number((CBYTE*)"BlackPoint", black, (CBYTE*)"Gamma");
	ENV_set_value_number((CBYTE*)"WhitePoint", white, (CBYTE*)"Gamma");
...
{
	*black = ENV_get_value_number((CBYTE*)"BlackPoint", 0, (CBYTE*)"Gamma");
	*white = ENV_get_value_number((CBYTE*)"WhitePoint", 256, (CBYTE*)"Gamma");
}
