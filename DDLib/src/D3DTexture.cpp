...
void NotGoingToLoadTexturesForAWhileNowSoYouCanCleanUpABit ( void )
{
	if ( pvFastLoadBuffer != NULL )
	{
		TRACE ((CBYTE*)"Freeing FastLoad buffer\n" );
		VirtualFree ( pvFastLoadBuffer, NULL, MEM_RELEASE );
...
HRESULT D3DTexture::Reload_TGA(void)
{
	...
	if (tga == NULL)
	{
		TRACE((CBYTE*)"Not enough MAIN memory to load tga %s\n", texture_name);
	...
	{
		TRACE((CBYTE*)"TGA %s is not square\n", texture_name);
		MemFree(tga);
		return DDERR_GENERIC;
	}
	...
	{
		TRACE((CBYTE*)"TGA %s is not a valid size", texture_name);
		MemFree(tga);
		return DDERR_GENERIC;
	}
	...
	if (!current_device)
	{
		TRACE((CBYTE*)"No device!\n");

		return DDERR_GENERIC;
	}

	TRACE((CBYTE*)"texture = %s\n", this->texture_name);
...
HRESULT D3DTexture::Reload_user()
{
	...
	if (best_mi == NULL)
	{
		...
		TRACE((CBYTE*)"Could not find texture format for the user texture page\n");
		return DDERR_GENERIC;
	}
	...
	if(FAILED(result))
	{
		TRACE((CBYTE*)"ReloadTextureUser: Could not get the texture interface.\n");
		Destroy();
		return DDERR_GENERIC;
	}
