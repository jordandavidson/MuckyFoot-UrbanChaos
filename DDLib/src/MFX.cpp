...
void MFX_init()
{
	...
	while (strcmp(names[0], "!"))
	{
		...
		if (_stricmp("null.wav", names[0]))
		{
			FILE*	fd = MF_Fopen(GetFullName(names[0]), "rb");
			if (fd)
			{
				sptr->fname = names[0];
				if (((!_strnicmp(names[0], "music",5))||(!_strnicmp(names[0], "generalmusic",12)))&&(!strstr(names[0],"Club1"))&&(!strstr(names[0],"Acid")))
				{
					sptr->is3D = false;
					sptr->type = SMP_Music;
				}
		...
		if (sptr->fname)
		{
			int gain = GetPrivateProfileIntA("PowerLevels", sptr->fname, 0, "data\\sfx\\powerlvl.ini");
			if (gain)
			{
				gain *= 4;
	...
	Volumes[SMP_Ambient] = float(ENV_get_value_number((CBYTE*)"ambient_volume", 127, (CBYTE*)"Audio")) / 127.0f;
	Volumes[SMP_Music] = float(ENV_get_value_number((CBYTE*)"music_volume", 127, (CBYTE*)"Audio")) / 127.0f;
	Volumes[SMP_Effect] = float(ENV_get_value_number((CBYTE*)"fx_volume", 127, (CBYTE*)"Audio")) / 127.0f;
}
...
void MFX_set_volumes(SLONG fx, SLONG amb, SLONG mus)
{
	...
	ENV_set_value_number((CBYTE*)"ambient_volume", amb, (CBYTE*)"Audio");
	ENV_set_value_number((CBYTE*)"music_volume", mus, (CBYTE*)"Audio");
	ENV_set_value_number((CBYTE*)"fx_volume", fx, (CBYTE*)"Audio");
}
